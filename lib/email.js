...
function methodLabel(method) {
  return { fawry: 'Fawry', vodafone_cash: 'Vodafone Cash', instapay: 'InstaPay' }[method] || method
}

// ── HTML Email Template ────────────────────────────────────────────────────────
function buildEmailHtml({ studentName, courseName, coursePrice, paymentMethod, registrationId, academyPhone, academyEmail, isConfirmed }) {
  ...
  const paymentInstructionsAr = {
    fawry:         `سيتم تحصيل رسوم التسجيل تلقائياً عبر فوري. توجّه لأقرب منفذ فوري وادفع برقم مرجع التسجيل.`,
  ...
  const paymentInstructionsEn = {
    fawry:         `Your registration fee will be collected automatically via Fawry. Visit any Fawry outlet and pay using your registration reference number.`,
