...
// Prevents XSS if a student puts <script> or <img onerror=...> in their name/course
...
function buildEmailHtml({ studentName, courseName, coursePrice, paymentMethod, registrationId, academyPhone, academyEmail, isConfirmed, whatsappGroupUrl, paymentReference, transactionId }) {
  ...
  const safeStudentName = escHtml(studentName)
  const safeCourseName  = escHtml(courseName)
  const safeCoursePrice = escHtml(coursePrice)
  const safePhone       = escHtml(academyPhone)
  const safeEmail       = escHtml(academyEmail)
  ...
  const displayRef = escHtml(paymentReference || transactionId || shortId)
  ...
  const paymentInstructionsAr = {
    ...
    vodafone_cash: `يُرجى إرسال المبلغ <strong>${safeCoursePrice} جنيه</strong> عبر فودافون كاش إلى رقم الأكاديمية، ثم أرسل صورة الإيصال عبر واتساب وسيتم تأكيد تسجيلك خلال 24 ساعة.`,
    instapay:      `يُرجى إرسال المبلغ <strong>${safeCoursePrice} جنيه</strong> عبر InstaPay إلى حساب الأكاديمية، ثم أرسل صورة الإيصال عبر واتساب وسيتم تأكيد تسجيلك خلال 24 ساعة.`,
  }
  ...
  const paymentInstructionsEn = {
    ...
    vodafone_cash: `Please send <strong>${safeCoursePrice} EGP</strong> via Vodafone Cash to the academy number, then send a screenshot via WhatsApp. Your registration will be confirmed within 24 hours.`,
    instapay:      `Please send <strong>${safeCoursePrice} EGP</strong> via InstaPay to the academy account, then send a screenshot via WhatsApp. Your registration will be confirmed within 24 hours.`,
  }
        ...
        <tr>
          <td style="padding:36px 40px;">
            ...
            <p style="margin:0 0 24px;font-size:17px;font-weight:700;color:#1A1A1A;direction:rtl;">
              مرحباً ${safeStudentName}، 👋
            </p>
            <p style="margin:0 0 28px;font-size:14px;color:#6B6B6B;line-height:1.7;direction:rtl;">
              ${isConfirmed
                ? `يسعدنا إعلامك بأن تسجيلك في كورس <strong style="color:#FF5C1A;">${safeCourseName}</strong> قد تم تأكيده بنجاح. نتطلع لرؤيتك معنا!`
                : `شكراً لتسجيلك في كورس <strong style="color:#FF5C1A;">${safeCourseName}</strong>. يُرجى اتباع تعليمات الدفع أدناه لاستكمال تسجيلك.`
              }
            </p>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="background:#FFF8F4;border-radius:14px;border:1px solid #FFE4D4;margin-bottom:28px;overflow:hidden;">
              ...
              <tr>
                <td style="padding:4px 0;">
                  ${row('📚 الكورس / Course',    safeCourseName)}
                  ${row('💳 طريقة الدفع / Payment', method)}
                  ${row('💰 الرسوم / Fee',          safeCoursePrice ? `${safeCoursePrice} جنيه` : '—')}
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="border-top:1px solid #FFE4D4;padding-top:24px;margin-top:4px;">
              <tr>
                <td style="padding-top:20px;">
                  ...
                  <p style="margin:0 0 6px;font-size:13px;color:#6B6B6B;">📞 ${safePhone || '+20 100 000 0000'}</p>
                  <p style="margin:0;font-size:13px;color:#6B6B6B;">✉️ ${safeEmail || 'info@artsmartacademy.com'}</p>
                </td>
              </tr>
            </table>
